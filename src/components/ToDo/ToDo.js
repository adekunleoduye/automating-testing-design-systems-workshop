...
function ToDoItem({ label, setTasks, tasks, setCompletedTasks, id }) {
  function handleOnChange(e) {
    setTasks((prevState) => {
      ...
      setCompletedTasks(
        newState.filter((task) => task.completed === true).length
      );
      return newState;
    });
  }
  ...
  const isCompleted = tasks.filter((task) => task.name === label)[0].completed;
  ...
  return (
    <Form.Group className={styles.rootItem} id={id}>
      <Form.Check
        ...
        id={label}
        label={
          <span className={cn(isCompleted && styles.isCompleted)}>{label}</span>
        }
...
function ToDo({ tasks = [] }) {
  ...
  const [currentTasks, setTasks] = useState(
    tasks.map((task) => ({ name: task, completed: false, id: uuid() }))
  );
        ...
        <InputGroup>
          ...
          <Button
            ...
            onClick={() => {
              ...
              setTasks([
                ...currentTasks,
                { name: value, completed: false, id: uuid() }
              ]);
              setValue("");
            }}
          >
        ...
        {currentTasks.length > 0 && (
          <footer className={styles.footer}>
            ...
            <div className={styles.progressWrapper}>
              <ProgressBar
                variant="success"
                now={(completedTasks / currentTasks.length) * 100}
              />
            </div>
          </footer>
        )}
